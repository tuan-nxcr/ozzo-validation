...
func (a Address) Validate() error {
	return validation.ValidateStruct(&a,
		...
		// City cannot be empty, and the length must between 1 and 200
		validation.Field(&a.City, validation.Required, validation.Length(1, 200)),
		...
		// Zipcode cannot be empty, and must be a string consisting of five or nine digits
		validation.Field(&a.Zipcode, validation.Required, validation.Match(regexp.MustCompile("^[0-9]{5}(?:-[0-9]{4})?$"))),
	)
}
...
err := validation.Validate(c,
	validation.Map(
		...
		validation.Key("Address", validation.Map(
			...
			// City cannot be empty, and the length must between 1 and 200
			validation.Key("City", validation.Required, validation.Length(1, 200)),
...
// Output:
// {"street":"the length must be between 1 and 200","state":"must be in a valid format"}
```
